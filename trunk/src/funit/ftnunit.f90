...
!
!     $Id: ftnunit.f90,v 1.10 2010-03-03 12:23:21 arjenmarkus Exp $
!
...
subroutine ftnunit_write_html_failed_real( text, value1, value2 )
    ...
    write( lun, '(a)' ) &
        '<td><span class="indent">Failed assertion: ', trim(text), '</span></td>', &
        '<td>Values: '
...
subroutine ftnunit_write_html_failed_real1d( text, idx, value1, value2, addtext )
    ...
    if ( addtext ) then
        write( lun, '(a)' ) &
            '<td><span class="indent">Failed assertion: ', trim(text), '</span></td></tr><tr>'
    endif
